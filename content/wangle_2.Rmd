...
gender_data <- filter(CIACountries, country %in% c("Chad", "Venezuela"))
gender_data
```
#### Practice with CIACountries
...
CIACountries %>% 
  ...
  select(country, roadways, net_users, area)
```
...
We can use other functions within mutate to help us make the new variable like `ifelse()`. If you have multiple conditions you can use the `case_when()` function and list out your possible options.
...
CIACountries %>% 
  mutate(oil_density = oil_prod / area)

example <- data.frame(name = c("Mike", "Franky", "Dylan"),
                      age = c(6, 7, 8),
...
CIACountries %>% 
  mutate(pop_group = case_when(
    pop < 100000000 ~ "Small",
    pop >= 10000000 & pop < 700000000 ~ "Medium to Large",
    pop >= 700000000 ~ "Large"
  )) %>% 
  group_by(pop_group) %>% 
  summarise(mean_edu = mean(educ))
```
